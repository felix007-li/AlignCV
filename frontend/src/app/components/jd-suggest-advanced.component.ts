...
export class JdSuggestAdvancedComponent implements OnInit {
  ...
  ngOnInit(){ const resume = this.store.selectSnapshot(ResumeState.entity); const exp = (resume?.blocks||[]).find((b:any)=>b.type==='experience'); this.bullets = splitBullets(exp?.data?.text || ''); const jdText = this.bullets.join(' '); this.store.dispatch(new AnalyzeJD(jdText)); (window as any).dataLayer=(window as any).dataLayer||[]; (window as any).dataLayer.push({ event:'ai_suggest_open', ab:'variant-A' }); }
