import { Component, OnInit, AfterViewInit, inject, signal, ViewChild, ElementRef, ViewEncapsulation } from '@angular/core';
...
export class LeftEditorComponent implements OnInit, AfterViewInit {
  ...
  private importService = inject(ResumeImportService);

  @ViewChild('profileEditor') profileEditorRef?: ElementRef<HTMLElement>;

  // Quill editor instances
  private profileQuill: Quill | null = null;
  private experienceQuills = new Map<string, Quill>();
  private educationQuills = new Map<string, Quill>();
   ...
   */
  private initProfileQuill() {
    if (!this.profileEditorRef) return;
   ...
   */
  private initEducationQuill(eduId: string) {
   ...
   */
  private initExperienceQuill(expId: string) {
  ...
  toggleExpSuggestions(id: string) {
  ...
  async getSuggestionsForProfile() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const suggestions = data.data?.suggestions?.map((s: any) => s.text) || [];
  ...
  async getSuggestionsForExperience(id: string) {
    const exp = this.experiences.find(e => e.id === id);
    if (!exp) return;
  ...
  applyProfileSuggestion(suggestion: string) {
  ...
  applyExpSuggestion(id: string, suggestion: string) {
    const exp = this.experiences.find(e => e.id === id);
    if (!exp) return;
  ...
  async onUpload(event: Event) {
    ...
    try {
      ...
      alert('Failed to import resume. Please try again. Error: ' + (error as Error).message);
    }
  ...
  private convertTextToHtml(text: string): string {
    ...
    const htmlLines = lines.map(line => {
      ...
      if (!trimmed) return '<p><br></p>'; // Empty line
