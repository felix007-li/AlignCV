...
export class ResumeImportService {
  ...
  private populateSections(text: string): void {
    ...
    const isSectionHeader = (line: string, keywords: RegExp): boolean => {
      if (!line || line.length > 60) return false;
      if (!keywords.test(line)) return false;
      // Avoid matching lines that start with lowercase or contain sentence patterns
      if (/^[a-z·]/.test(line)) return false; // Starts with lowercase or bullet
      if (/\s(in|with|for|and|the|of|to)\s/i.test(line) && line.length > 30) return false; // Contains common sentence words
      return true;
    };
    ...
    const findNextSectionIndex = (linesArray: string[]): number => {
    ...
    if (expIndex !== -1) {
      ...
      const jobEntries: Array<{ position: string; employer: string; description: string }> = [];
      let i = 0;
    ...
    if (eduIndex !== -1) {
      ...
      const eduEntries: Array<{ degree: string; school: string; description: string }> = [];
      let i = 0;
