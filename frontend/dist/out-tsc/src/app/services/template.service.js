...
let TemplateService = class TemplateService {
     ...
     */
    getTemplateById(id) {
        return this.getAllTemplates().pipe(map(templates => templates.find(t => t.metadata.id === id)));
    }
    /**
     * Get templates filtered by industry tag
     */
    getTemplatesByIndustry(industry) {
        return this.getAllTemplates().pipe(map(templates => templates.filter(t => t.metadata.industry_tags.includes(industry))));
    }
    /**
     * Get templates filtered by style tag
     */
    getTemplatesByStyle(style) {
        return this.getAllTemplates().pipe(map(templates => templates.filter(t => t.metadata.style_tags.includes(style))));
    }
    /**
     * Get paginated templates
     */
    getTemplatesPaginated(page = 0, pageSize = 10) {
        return this.getAllTemplates().pipe(map(allTemplates => {
