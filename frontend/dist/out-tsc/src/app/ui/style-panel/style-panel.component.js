...
import { SetTemplate, SetFont, SetFontSize, SetLineHeight, SetPalette } from '../../state/ui.state';
...
let StylePanelComponent = class StylePanelComponent {
    constructor() {
        this.store = inject(Store);
        this.styleSync = inject(StyleSyncService);
        this.uiState$ = this.store.select((state) => state.ui);
        this.uiStateSnapshot = null;
        this.defaults = { template: 'sans', font: 'Arial, sans-serif', fontSize: 14, lineHeight: 1.25, palette: 'blue' };
        this.palettes = [
            { key: 'blue', name: 'Blue', value: '#2563eb' },
            { key: 'green', name: 'Green', value: '#059669' },
            { key: 'rose', name: 'Rose', value: '#e11d48' },
            { key: 'purple', name: 'Purple', value: '#7c3aed' },
            { key: 'orange', name: 'Orange', value: '#ea580c' },
        ...
        this.userSelectedColor = null; // Track user-selected color
        ...
        this.currentFontLabel = 'Arial';
        this.lineHeightOptions = [1, 1.15, 1.25, 1.5, 2];
        this.currentLineHeightLabel = '1.25';
    }
    ngOnInit() {
    ...
    updateColorFromPalette(palette) {
        const paletteColors = {
            blue: '#2563eb',
            green: '#059669',
            rose: '#e11d48',
    ...
    setPreviewVar(key, value) {
        const previewElement = document.querySelector('.resume-preview');
        const canvasElement = previewElement?.querySelector('.resume-canvas');
        if (previewElement)
            previewElement.style.setProperty(key, value);
        if (canvasElement)
            canvasElement.style.setProperty(key, value);
    }
    applyStateToPreview(state) {
        const paletteColors = {
            blue: '#2563eb',
            green: '#059669',
            rose: '#e11d48',
    ...
    applyTemplateTokens(template) {
        ...
        if (this.userSelectedColor) {
            setTimeout(() => {
                ...
                const previewElement = document.querySelector('.resume-preview');
                if (previewElement) {
                    previewElement.style.setProperty('--color-primary', this.userSelectedColor);
                }
